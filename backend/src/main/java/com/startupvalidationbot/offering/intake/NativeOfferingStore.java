        ...
        if (existing.isPresent()) {
            ...
            jdbc.update("""
                    UPDATE radar_offerings SET radar_company_id=COALESCE(?,radar_company_id),
                      ...
                      maximum_amount=CASE WHEN provenance='SEC_EDGAR' THEN maximum_amount ELSE COALESCE(?,maximum_amount) END,
                      valuation_or_cap=CASE WHEN provenance='SEC_EDGAR' THEN valuation_or_cap ELSE COALESCE(?,valuation_or_cap) END,
                      deadline=CASE WHEN provenance='SEC_EDGAR' THEN deadline ELSE COALESCE(?,deadline) END,
                      amount_raised=CASE WHEN provenance='SEC_EDGAR' THEN amount_raised ELSE COALESCE(?,amount_raised) END,
